...
//		for (int i=1;i==150;i++) {
//			this.setSpeed(i);
//			System.out.println(this.model + "speedUp" + "시속:" + this.speed+"km/h");
//		}
	}//run method end
...
public class classKeyword {
	...
	public static void main(String[] args) {
		...
		CarTest car2 = new CarTest("벤츠S클래스");
		...
		// 문제
		// 붕어빵을 만드는 FishBreadUp 이라는 클래스를 생성해주세요
		// 메인 클래스 이름(main 메서드가 들어올 함수) FishMain를 생성해주세요
		
		// FishBreadUp 클래스에서 클래스변수 문자형 변수 cream을 선언해주세요.
		
		// FishBreadUp 클래스를 통해 인스턴스를 마음껏 생성해보세요.
		// 생성한 인스턴스를 통해 cream 필드를 채워 넣으시고
		// 만들고 싶은 붕어빵들을 완성해주세요.
		
		// static(고정)
		// 클래스에 고정된 멤버로서 객체를 생성하지 않고 사용할 수 있는 필드와 메서드.
		// 클래스에 소속된 멤버(메서드 영역)이기 때문에 클래스 멤버라고도 부른다.
		// 접근 지정자 선언후 사용하거나 타입 지정전 사용
		// 클래스의 로딩이 끝난후에 사용이 가능하다
		
		// 클래스에서 어떠한 필드에 static을 선언한다 = 해당 클래스가 생성할 객체에서
		// 해당 필드는 모두 공통된 값
		
		// 클래스 메서드(static)
		// 인스턴스 메서드 : 메서드 작업 수행시 (메서드 실행시) 필드를 필요로 하는 메서드.
		// static 메서드 : 인스턴스와 관계 없는 메서드를 클래스 메서드(static메서드)로 정의
		// - 인스턴스 변수(클래스 필드)가 필요 없는 메서드 생성시.
		// - 클래스 메서드(static 메서드)의 경우는 인스턴스 생성없이 호출이 가능하다.
		// - 클래스 메서드 호출시 인스턴스가 존재할 수도 하지 않을수도 있음.
		...
		// 상수(constant) - 불변의 값
		// 불변의 값을 저장하는 필드를 자바에서 상수라 부른다.
		// 상수의 개념을 듣게되면 final 키워드가 적용된 필드를 상수로 인식할 수 있다.
		// 하지만 final 필드는 객체마다 저장이 된다는 점, 그리고 생성자의
		// 파라미터를 통해서 다른 값을 가질 수가 있음.
		...
		// 코드의 가독성을 위해서 상수는 모두 대문자로 작성하는 것이 개발자들끼리의 관례
